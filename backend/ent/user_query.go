...
func (uq *UserQuery) QueryScholarshipRequest() *ScholarshipRequestQuery {
	query := &ScholarshipRequestQuery{config: uq.config}
...
func (uq *UserQuery) WithScholarshipRequest(opts ...func(*ScholarshipRequestQuery)) *UserQuery {
	query := &ScholarshipRequestQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withScholarshipRequest = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withScholarshipRequest; query != nil {
		...
		query.Where(predicate.ScholarshipRequest(func(s *sql.Selector) {
			s.Where(sql.InValues(user.ScholarshipRequestColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_scholarship_request
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_scholarship_request" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_scholarship_request" returned %v for node %v`, *fk, n.ID)
			}
